...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # ── 2. Groups (import from CSV if missing) ──
        ...
        # ── 19. Articles ──
        articles_specs = [
            # (designation, produit_idx, marque_idx, fournisseur_idx, nature_idx, prix, qte, facture)
        ...
        # ── 20. Items (one per unit in each article, up to a cap) ──
        items = []
        tag_idx = 0
        for art in articles:
            count = min(art.qte or 1, 8)  # cap at 8 items per article
            for j in range(count):
                emp = random.choice(emps[:15])  # main sites
        ...
        archive_objs = []
        archive_data = [
            ("Hors service - carte mere defaillante", "reforme"),
            ("Obsolete - remplacement prevu", "reforme"),
            ("Endommage suite a chute", "casse"),
            ("Fin de vie - amortissement complet", "reforme"),
            ("Retour fournisseur - defaut fabrication", "retour_fournisseur"),
            ("Desaffecte suite a reorganisation", "autre"),
            ("Panne irreparable", "casse"),
            ("Mis au rebut", "reforme"),
            ("Don a association", "don"),
            ("Vol declare", "vole"),
        ]
        for i, it in enumerate(archived):
            ...
            it.save()
            commentaire, motif = archive_data[i % len(archive_data)]
